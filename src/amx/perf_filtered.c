...
static const perfcase_t perfcases[] = {
    ...
    {"fma16_vec", 1ull << 63, 1, 0x3f, FLAG_VEC, 15, fma16_width_modes_vec,
     fma_alu_modes},
    {"fma32_vec", 1ull << 63, 1, 0x3f, FLAG_VEC, 12, fma32_width_modes,
     fma_alu_modes},
    {"fma64_vec", 1ull << 63, 1, 0x3f, FLAG_VEC, 10, fma64_width_modes,
     fma_alu_modes},

    {"mac16_mat", 0, 32, 0x01, FLAG_INT, 14, mac16_width_modes_mat,
     fma_alu_modes},
    {"mac16_vec", 1ull << 63, 1, 0x3f, FLAG_INT | FLAG_VEC, 14,
     mac16_width_modes_vec, fma_alu_modes},
    ...
    {"vecint", 0, 1, 0x3f, FLAG_INT | FLAG_VEC, 18, vecint_width_modes,
     vecint_alu_modes},
    {},
};
...
static uint8_t detect_hardware_m2_flag() {
  ...
  AMX_LDX(PTR_ROW_FLAGS(
      buf, 16,
      1)); // On M1: copy buf[0:128] to x[0:2], on M2: copy buf[0:256] to x[0:4]
...
int main() {
  ...
  printf("test_name,z_spacing,num_z,bytes_per_z,num_threads,insn_total,ops_"
         "total,ns_elapsed,ops_unit\n");
  thread_context_t ctx;
  for (const perfcase_t *perfcase = perfcases; perfcase->name; ++perfcase) {
    if (strcmp(perfcase->name, "matfp"))
      continue;
    for (const perfmode_t *width_mode = perfcase->width_modes; width_mode->name;
         ++width_mode) {
      for (const perfmode_t *alu_mode = perfcase->alu_modes; alu_mode->name;
           ++alu_mode) {
        if (strcmp(width_mode->name, "f16f16") &&
            strcmp(width_mode->name, "f16f32") &&
            strcmp(width_mode->name, "f32f32"))
          continue;
        uint8_t flags = perfcase->flags | width_mode->flags | alu_mode->flags;
        if (flags & ~allowed_flags)
          goto next_mode;
        uint8_t z_mask =
            perfcase->z_mask & width_mode->z_mask & alu_mode->z_mask;
        uint32_t max_num_z = 1u << __builtin_popcount(z_mask);
        uint32_t bytes_per_z = 4096 / max_num_z;
        if (max_num_z > 16) {
          max_num_z = 16;
        }
        uint32_t ops_per_insn = (uint32_t)perfcase->op_scale *
                                (uint32_t)width_mode->op_scale *
                                (uint32_t)alu_mode->op_scale;
        ...
        for (uint32_t n_z = 1; n_z <= max_num_z; ++n_z) {
          ...
          for (uint32_t far_z = 1; far_z <= 1; ++far_z) {
            uint64_t encoding =
                perfcase->encoding ^ width_mode->encoding ^ alu_mode->encoding;
            for (uint32_t z = 0; z < n_z; ++z) {
              ctx.args[z] = encoding;
              encoding =
                  (far_z ? add_one_using_bitmask_hi : add_one_using_bitmask_lo)(
                      encoding, ((uint64_t)z_mask) << 20);
            }
            for (uint32_t n_threads = 1; n_threads <= 1; ++n_threads) {
              ctx.count = op_count_target / (ops_per_insn * n_threads * n_z);
              ctx.count = (ctx.count + 4) & ~(uint64_t)7;
              uint64_t t0 = now(CLOCK_REALTIME);
              dispatch_apply_f(n_threads, DISPATCH_APPLY_AUTO, (void *)&ctx,
                               thread_func);
              ...
              printf("%s_%s_%s,%s,%u,%u,%u,%llu,%llu,%llu,%s\n", perfcase->name,
                     width_mode->name, alu_mode->name, far_z ? "far" : "near",
                     (unsigned)n_z, (unsigned)bytes_per_z, (unsigned)n_threads,
                     (long long unsigned)insn_total,
                     (long long unsigned)(insn_total * ops_per_insn),
                     (long long unsigned)wall_elapsed,
